...
/**
 * The "step out to the full feature" link every capture tab carries (e.g. "See full
 * competency evidence") — a deliberate exit from the shift-scoped view to the
 * standalone section. Navigating away closes the modal.
 */
export function SeeFullLink({ to, children }: { to: string; children: ReactNode }) {
  return (
    <Link
      to={to}
      className="inline-flex shrink-0 items-center gap-1 whitespace-nowrap text-xs font-medium text-secondary-700 transition hover:text-secondary-800 hover:underline"
    >
      {children}
      <svg
        viewBox="0 0 24 24"
