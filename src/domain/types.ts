...
export interface ProficiencyStatusEvent extends Entity, Created {
  ...
  /** Provenance when this wasn't typed by hand (spec-note-capture.md P5). */
  sourceType?: SourceType;
  sourceId?: string; // the `NoteBlock.id` this was materialised from
}
...
// ---------- Note capture (photos of handwritten notes) ----------

/**
 * Where a row came from, when it wasn't typed by hand (spec-note-capture.md P5).
 * Discriminated like `EvidenceLink.evidenceType` so later sources (voice, import) can be
 * added without a backfill or a guess at what existing ids point to.
 */
export type SourceType = "NOTE_BLOCK";

export type NoteCaptureStatus = "PARSING" | "REVIEW" | "DONE";
export const NOTE_CAPTURE_STATUS_LABEL: Record<NoteCaptureStatus, string> = {
...
/**
 * What a block appears to be. The vision model offers this as a *hint*; the classifier
 * decides the stored value (P26/P30). `UNKNOWN` is deliberate and honest — a block the
 * classifier couldn't type is shown as such and retyped by the student (P34), never
...
export const NOTE_BLOCK_TARGET_LABEL: Record<NoteBlockTarget, string> = {
  REFLECTION: "Reflection",
  MED_LOG: "Medication log",
  PROFICIENCY_EVENT: "Proficiency evidence",
  SHIFT_NOTES: "Shift notes",
};

/**
 * One photographed page (or several — a capture is a notebook session, P20).
 * The image itself lives in S3 and is kept for the life of the account (P13); this row
 * is the evidence chain's middle link: domain row → `NoteBlock` → `NoteCapture` → S3.
 */
export interface NoteCapture extends Entity, UserOwned, Created, Updated {
  shiftId?: string; // suggested shift for the page (P6/P9)
  pageDateRaw?: string; // date exactly as written, e.g. "22/7" — never normalised (P8)
  /** Comma-separated S3 object keys in upload order. Same idiom as `AiMessage.noteRefs`;
   *  safe because keys are generated by us and never contain a comma. */
  imageKeys: string;
  /** The standing "no patient-identifiable information" acknowledgement (P2), recorded
   *  per capture so it's provable rather than a dialog nobody can evidence. */
  piiAcknowledged: boolean;
  status: NoteCaptureStatus;
}

/** A capture to create — the store stamps id + timestamps. */
export type NoteCaptureDraft = Omit<NoteCapture, "id" | "userId" | "createdAt" | "updatedAt">;

/**
 * A **semantic** unit of a photographed page (P26) — not a visual region. The vision
 * model's regions are guidance about where the subject changes; a block may span two or
 * three of them, or subdivide one.
 *
 * Three text states, no revision history (P11): `rawText` is the verbatim transcription,
 * frozen at parse; `text` is the sanitised version (P24) the student then edits. Keeping
 * `rawText` is what makes the sanitiser safe to auto-apply — every correction stays
 * diffable against what the page actually said.
 */
export interface NoteBlock extends Entity, UserOwned, Created, Updated {
  captureId: string; // FK → NoteCapture
  imageIndex: number; // which photo within the capture (P20)
  /** Comma-separated vision-region indices this block drew from (P26). */
  fromRegions?: string;
  rawText: string; // verbatim transcription — frozen at parse (P11)
  text: string; // sanitised (P24), then student-edited
  /** Comma-separated `from|to` pairs the sanitiser applied (P24), surfaced in review so
   *  any correction is one tap to revert. */
  corrections?: string;
  /** Comma-separated `structureReading|checkReading` pairs the two vision models
  ...
  confidence: number;
  bboxX0: number; // 0–1 fractions (the model returns 0–1000; normalise on ingest)
  bboxY0: number;
  bboxX1: number;
  bboxY1: number;
  rotationDeg: number;
  /** Comma-separated taxonomy codes, best first, e.g. "B2.1,B2.4,3.4" (P28). The first is
   *  pre-selected; the rest are one tap away. Absent for kinds with no taxonomy target. */
  candidateCodes?: string;
  /** Comma-separated tag labels (P37). Labels already in the student's `Tag` list are
   *  applied; genuinely new ones need confirming. */
  ...
  targetId?: string; // the row allocation created
  appendedTo?: string; // Shift.id, when allocated to SHIFT_NOTES (P7)
  /** Exactly what was appended, so un-allocating can strip it back out (P19). */
  appendedText?: string;
}

/** A block to create — the store stamps id + timestamps. */
export type NoteBlockDraft = Omit<NoteBlock, "id" | "userId" | "createdAt" | "updatedAt">;
