...
| real-mindmap-sepsis | **PARTIAL** | **One DIAGRAM block synthesised ✓** — central node + spokes 3–6, union bbox, `diagram` tag, no target (keep/dismiss only). Content also present as its own blocks ✓; margin notes separate ✓; BUFFALO read correctly ✓. Misses: vision's membership left spokes 1–2 out and pulled the red-flag margin note in; the immunosuppression ask typed CLINICAL_SKILL, not TODO. |
