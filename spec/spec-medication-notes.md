...
- **Add / edit medication** → triggers a generic **calc drill** on first add. Fields
  with a known vocabulary offer **type-ahead, substring-match autocomplete**. Generic
  **name**, **drug class** and **body system** draw on **real NHSBSA BNF open data**
  (Open Government Licence v3.0 — chemical substances ≈ generic names, sections +
  paragraphs ≈ drug classes, chapters ≈ body systems; ~2,500 names / ~450 classes).
  It's bundled at build time into `src/data/bnf.ts` (regenerated by
  `scripts/fetch-bnf.py`, which downloads the latest "BNF Code Information" CSV) so the
  app stays offline. **Side effects** + **monitoring** (multi-value chip inputs) and
  **routes** are **curated controlled vocabularies** — the BNF monograph text those
  would come from is proprietary (the NICE BNF API is org-licensed), so there's no free
  source. Every field still accepts free text; provenance (`BNF_SOURCE`) is shown on
  the form. A study aid — not a clinical reference. (Replaces the old native
  `<datalist>`, which only prefix-matched and rendered inconsistently.)
