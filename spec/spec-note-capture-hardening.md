# Note capture — wrap-up & hardening spec  (Status: PHASES 2–3 (H1–H3, H7–H9) BUILT + verified on dev; H4/H5 in flight elsewhere; H6, H10–H12 next)
...
3. **Hardening** (H7, H8, H9). Gate: kill the network mid-upload and mid-parse in the browser → both recover; parse counter visible in DynamoDB and enforced. **DONE 2026-08-05** — gate met on dev: two dropped presigns showed "The connection dropped. Getting ready to send your photo again — attempt 3 of 3" and the upload then landed; a reload mid-parse left a capture `PARSING` with 0 blocks, and re-opening the Photo button resumed it for free from `parse.json` (5 blocks persisted, status `REVIEW`, nothing left parsing); `DAILY#PARSE#2026-08-05` appeared in DynamoDB alongside `DAILY#PHOTO#…` and its per-page marker row, and parking it at 30 produced the calm cap screen from the deployed Lambda. One addition beyond the spec: **the photo count is claimed per page per day**, because retrying a presign whose reply was lost would otherwise spend two or three of the student's ten — H7 can't be honest without it. Emergent and correct: a capture the parse cap interrupted is also left `PARSING`, so it resumes free from cache on the next open rather than rotting.
4. **Performance** (H10–H12). Gate: a two-page capture's wall-clock beats sequential by roughly one upload+downscale; main thread stays responsive during downscale (no long tasks > 100ms from the encode). **DONE 2026-08-05** — gate met on dev with two real reads: page 1's presign started at +15479 ms and its PUT finished at +16091 ms, entirely inside page 0's parse (which began at +15480 ms), so the second page's decode, encode, hash, presign and upload are all off the critical path. Both pages went through the worker (2 messages, 2 replies) and produced the same 16 blocks as the main-thread path. **Honest limit on the H11 measurement:** on an M-series Mac the encode blocks the main thread for ~30 ms either way, so worker-vs-fallback is indistinguishable here (0 blocks over 100 ms on both) — the jank H11 removes is a phone-class problem, and what's verified on this hardware is that the work runs off-thread and the output is byte-identical, not the size of the saving. (Also: `setTimeout` is clamped to ~1 s while the browser pane is hidden, which measures the clamp rather than the block — the responsiveness figures above come from a MessageChannel ticker.)
