...
| # | Decision |
|---|----------|
| D6a | **Interim-model amendment (2026-07-26).** The account is temporarily blocked from creating the Anthropic model agreement (AWS support case open), so v1 builds against the **Bedrock mantle endpoint** (`bedrock-mantle.eu-west-2.api.aws`, plain IAM SigV4 — verified working) with a **provider adapter**: OpenAI-compat route + an open-weight interim model (candidate picked by the eval harness from DeepSeek v3.2 / Kimi K2.5 / GLM-4.7 / MiniMax M2.5) now; Anthropic-native route + `anthropic.claude-sonnet-5` (D6, with prompt caching) the moment the agreement unblocks. Model + provider are config; the swap requires an eval rerun only. Interim path has **no prompt caching** (open-weight pricing keeps questions <1¢ regardless). Confidence gate runs tighter on the interim model. **Whether beta students ever see the interim model is decided at Gate 5** from eval transcripts; the D14 launch email ships with whichever model launches. |
