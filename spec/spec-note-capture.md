...
Numbered P1–P43 in decision order; P26–P40 were added after grilling the classification
...
| P43 | **A drawn diagram is a block kind of its own, kept rather than filed** (added 2026-08-04, prompted by the first mind-map page in the real corpus — `tests/pages/`). A mind map, flowchart or sketch becomes ONE `DIAGRAM` block whose `text` transcribes the drawing's words in reading order and whose bbox is the **union** of every region it spans; the classifier ALSO emits normal blocks for the same content so the notes file wherever they belong — the duplication is deliberate, text-in-the-right-place AND the drawing kept whole. A diagram has no filing target: review offers keep-or-dismiss, and keeping sets the new `NoteBlockStatus` **`KEPT`** — settled like filed, but claiming no created row, because the retained photo IS the artefact. Kept diagrams are the one `NoteBlock` the recall corpus reads directly (`[DIAGRAM:id]`), since no domain row will ever carry their words; filed blocks stay excluded to avoid double-counting. The invented-content guard relaxes to **word-level** containment for DIAGRAM only — a faithful transcription reorders scattered labels, but still may not introduce a word the page doesn't have. |
