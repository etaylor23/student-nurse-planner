...
Numbered P1–P45 in decision order; P26–P40 were added after grilling the classification
...
| P41 | **Content-addressed storage with a cached parse.** A page's identity is the SHA-256 of its **downscaled** bytes, so it lives at `u/<sub>/h/<hash>/page.jpg` and its finished parse is written beside it as `parse.json`. Re-uploading a page this student has already read returns the stored parse instead of a PUT URL: no upload, no four model calls, no ~70-second wait, and **no charge against the daily cap** (P17) — nothing happened that a cap exists to limit, and a page you have already read should still open on a day you have used up. What is cached is the parse **as first presented**, before any editing, retyping or filing, because those live on the student's `NoteBlock` rows and not in the parse. The review screen says so plainly and offers to read the page again from scratch, which overwrites the cache (same key). Two properties make this safe rather than clever: the hash is over the *downscaled* bytes, so a hit can only ever be the exact image the models saw; and the key includes `sub`, so one student's parse can never be served to another. A cache that can't be read degrades to a normal parse. `captureId` is deliberately absent from the path — a page's identity is its bytes, not the session that uploaded it. |
| P42 | **A block can be dismissed.** Not everything on a page deserves a row: a page title (`Medication notes`), a phone number, a stray line. Dismissing soft-deletes the `NoteBlock` through the normal repository path so it tombstones and syncs, in two taps rather than one. This does **not** contradict P34's "nothing the student wrote is ever discarded": the photo is retained for the life of the account (P13) and the parse is cached against its hash (P41), so re-reading the page brings the block straight back at no cost. Already-filed blocks can't be dismissed — the real row exists, so the filing has to be undone first (P19). |
| P43 | **A drawn diagram is a block kind of its own, kept rather than filed** (added 2026-08-04, prompted by the first mind-map page in the real corpus — `tests/pages/`). A mind map, flowchart or sketch becomes ONE `DIAGRAM` block whose `text` transcribes the drawing's words and whose bbox is the **union** of every region it spans; normal blocks ALSO carry the same content so the notes file wherever they belong — the duplication is deliberate, text-in-the-right-place AND the drawing kept whole. A diagram has no filing target: review offers keep-or-dismiss, and keeping sets the new `NoteBlockStatus` **`KEPT`** — settled like filed, but claiming no created row, because the retained photo IS the artefact. Kept diagrams are the one `NoteBlock` the recall corpus reads directly (`[DIAGRAM:id]`), since no domain row will ever carry their words; filed blocks stay excluded to avoid double-counting. **The DIAGRAM block is synthesised in code** (`diagram.ts`), not model-emitted: the VISION model nominates the drawing's regions (per-region DIAGRAM hints + shared groupKey, largest cluster, unioned with any classifier `diagramRegions`), and the block's text is the nominated regions' own transcriptions with the sanitiser's validated swaps replayed. Both prompt-level alternatives were run against the real page and failed in opposite directions — and a direct probe proved the text-only classifier (P12) answers `diagramRegions: []` because it cannot see arrows. Vision saw the pixels; vision nominates. The invented-content guard relaxes to **word-level** containment for model-emitted DIAGRAM blocks only. |
| P44 | **The drawing is rebuilt as Mermaid and rendered in the UI** (retrospective, built 2026-08-04). The structure vision call also returns a top-level `diagramMermaid` — a Mermaid rebuild of the drawing (`mindmap` for a mind map, `flowchart` for boxes-and-arrows) using only the page's written words. **Folded into the existing vision call by explicit decision** (no fifth model call, no extra latency or cost); a **separate conditional vision call remains a documented, doable alternative** — fire it only when a diagram was nominated — if the fold ever destabilises the main transcription call the way small prompt additions have before (the `groupKey: null` incident). Admitted through `guardMermaid`: structure (nesting, arrows, node ids) is legitimately model-invented, words are not — any word not on the page rejects the whole source. Stored as `NoteBlock.diagramSource`, rendered by `MermaidDiagram.tsx` on the review DIAGRAM card and in ask-your-notes DIAGRAM quotes: `mermaid` is lazy-imported (~1.5 MB stays out of the app bundle), `securityLevel: "strict"`, and rendering **fails closed** — an unparseable source shows nothing while the transcription stays, so a bad rebuild costs a picture, never content. |
| P45 | **A page can hold several drawings** (retrospective, built 2026-08-04 for the first two-drawing corpus page — a mind map AND a branching flowchart). The vision call returns `diagrams[]` (one entry per drawing: its groupKey, its form, its Mermaid — flowchart decision answers ride the arrows as edge labels, `C -- YES --> D`), and one DIAGRAM block is synthesised per cluster instead of largest-cluster-wins; the classifier's flat nomination folds into the cluster it overlaps most, or stands alone. In review, the left column pins a **"Selected drawing"**: tabs when there are two or more, auto-following focus (opening a drawing or any of its branches selects it) while an unrelated block keeps the current selection rather than blanking the pane; narrow screens stay per-focus with no tabs. Two reliability lessons this page taught: the mermaid word-guard must allow **bare node-id references** (`C -- YES --> D` references ids declared elsewhere — treating them as invented words rejected every real flowchart), and the structure model **intermittently returns a tiny partial read on dense pages** (3 of 6 runs collapsed to ~2 regions), now caught by comparing volume against the check model's whole-page text and retrying the structure call once (`structure.retried` in diagnostics). |
